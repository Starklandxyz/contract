...
mod go_fight {
    ...
    // 根据MapId 获取地址，已经获取到对方玩家信息、计算双方人数
        ...
        let win_rate_x = x * 10000 / (actual_attack_power_y + x * 100) /100; // x 放大 10000 和胜负率类似
        let random_loss_x = rand::thread_rng().gen_range(0..(100 - win_rate_x)) * x / 100;
    
        let win_rate_y = actual_attack_power_y *10000 / (actual_attack_power_y + x * 100)/100;
